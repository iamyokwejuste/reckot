...
def dashboard_callback(request, context):
    ...
    orders_last_7 = Booking.objects.filter(
        created_at__gte=last_7_days
    ).count()
...
def generate_sales_chart(start_date, end_date):
    ...
    return fig.to_html(include_plotlyjs='cdn', div_id="sales_chart", config={'displayModeBar': False})
...
def revenue_analytics(request):
    ...
    total_revenue_raw = payments.aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00')
    total_fees_raw = payments.aggregate(Sum('service_fee'))['service_fee__sum'] or Decimal('0.00')
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Revenue Over Time', 'Fee Breakdown', 'Net Revenue', 'Payment Status'),
        specs=[[{'type': 'scatter'}, {'type': 'pie'}],
               [{'type': 'bar'}, {'type': 'pie'}]]
    )

    fig.add_trace(
        go.Scatter(x=dates, y=revenues, name='Total Revenue', line=dict(color='#a78bfa', width=3)),
        row=1, col=1
    )

    fig.add_trace(
        go.Pie(labels=['Service Fees', 'Net Revenue'],
    ...
    fig.add_trace(
        go.Bar(x=dates, y=net, name='Net Revenue', marker_color='#34d399'),
        row=2, col=1
    )
    ...
    fig.add_trace(
        go.Pie(labels=[p['status'] for p in payment_status],
...
def event_analytics(request):
    events = Event.objects.annotate(
        revenue=Sum('bookings__payment__amount', filter=Q(bookings__payment__status='CONFIRMED')),
    ...
    fig.add_trace(
        go.Bar(y=event_names, x=event_revenues, orientation='h',
               marker_color='#a78bfa', name='Revenue'),
        row=1, col=1
    )

    fig.add_trace(
        go.Bar(y=event_names, x=event_tickets, orientation='h',
               marker_color='#ec4899', name='Tickets'),
        row=1, col=2
    )
...
def ticket_analytics(request):
    ...
    tickets = Ticket.objects.filter(
        booking__created_at__date__gte=last_30_days
    ).annotate(
    ...
    not_checked_in = Ticket.objects.filter(checked_in_at__isnull=True, booking__status='CONFIRMED').count()
    ...
    fig.add_trace(
        go.Scatter(x=dates, y=counts, mode='lines+markers',
                   line=dict(color='#a78bfa', width=3), name='Tickets Sold',
                   marker=dict(size=8, color='#a78bfa')),
        row=1, col=1
    )

    fig.add_trace(
        go.Pie(labels=['Checked In', 'Not Checked In'],
...
def payment_analytics(request):
    ...
    total_volume_raw = payments.filter(status='CONFIRMED').aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00')
    failed_volume_raw = payments.filter(status='FAILED').aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00')
    pending_volume_raw = payments.filter(status='PENDING').aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00')
    ...
    payments_by_gateway = payments.filter(status='CONFIRMED').values('provider').annotate(
        volume=Sum('amount'),
        count=Count('id')
    )

    gateways = [p['provider'] for p in payments_by_gateway]
    gateway_volumes = [float(convert_to_usd(p['volume'], 'XAF')) for p in payments_by_gateway]
    gateway_counts = [p['count'] for p in payments_by_gateway]

    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Payment Volume by Gateway', 'Transaction Count by Gateway',
                       'Payment Status Distribution', 'Processing Fees Over Time'),
        specs=[[{'type': 'bar'}, {'type': 'bar'}],
    ...
    fig.add_trace(
        go.Scatter(x=fee_dates, y=fee_amounts, mode='lines+markers',
                   line=dict(color='#fbbf24', width=3), name='Service Fees',
    ...
    service_fees_usd = convert_to_usd(total_fees_raw['service'] or Decimal('0.00'), 'XAF')

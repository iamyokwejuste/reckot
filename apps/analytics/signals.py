...
def update_payment_metrics(sender, instance, created, **kwargs):
    if instance.status == "CONFIRMED":
        ...
        if created:
            ...
            daily_metrics.net_revenue = daily_metrics.total_revenue - daily_metrics.platform_fees
            daily_metrics.orders_count += 1
            daily_metrics.save()

        if hasattr(instance, 'booking') and instance.booking and instance.booking.event:
            event_metrics, _ = EventMetrics.objects.get_or_create(event=instance.booking.event)
            ...
            event_metrics.net_revenue = event_metrics.total_revenue - event_metrics.platform_fees
            event_metrics.save()
            ...
            org_metrics.net_revenue = org_metrics.total_revenue - org_metrics.total_fees_paid
            org_metrics.save()
        ...
        if created:
            PaymentMetrics.objects.create(
                ...
                net_amount_to_organizer=instance.amount - (instance.service_fee or Decimal("0.00"))
            )
...
def update_booking_metrics(sender, instance, created, **kwargs):
    if created:
        ...
        if instance.event:
            ...
            event_metrics.orders_count = Booking.objects.filter(event=instance.event).count()
            event_metrics.save()

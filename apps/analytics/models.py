...
class EventMetrics(models.Model):
    event = models.OneToOneField(
        "events.Event", on_delete=models.CASCADE, related_name="metrics"
    )
    total_revenue = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("0.00")
    )
    ...
    def calculate_conversion_rate(self):
        if self.unique_visitors > 0:
            self.conversion_rate = (
                Decimal(self.orders_count) / Decimal(self.unique_visitors)
            ) * Decimal("100")
        else:
...
class PaymentMetrics(models.Model):
    payment = models.OneToOneField(
        "payments.Payment", on_delete=models.CASCADE, related_name="metrics"
    )
    ...
    net_amount_to_organizer = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("0.00")
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class OrganizationMetrics(models.Model):
    organization = models.OneToOneField(
        "orgs.Organization", on_delete=models.CASCADE, related_name="metrics"
    )
    total_revenue = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("0.00")
    )

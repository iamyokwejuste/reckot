...
class AIQueryService:
    ...
    def generate_sql(
        ...
        for table_name, table_schema in self.validator.tables.items():
            if self.validator._has_access(access_level, table_schema["access_level"]):
                columns = self.validator.get_accessible_columns(
        ...
        prompt = f"""Access: {access_level} | Schema: {schema_context} | Question: {user_question} | Generate PostgreSQL SELECT query only (no *, no writes, include row_filter WHERE clauses):"""
    ...
    def answer_question(self, user_question: str, user=None) -> Dict[str, Any]:

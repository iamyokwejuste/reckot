...
class EventAdmin(admin.ModelAdmin):
    list_display = ['title', 'organization', 'state', 'is_public', 'is_featured', 'feature_status', 'start_at']
    ...
    fieldsets = (
        (None, {
            'fields': ('organization', 'title', 'slug', 'description', 'short_description', 'cover_image')
        }),
        ('Event Details', {
            'fields': ('event_type', 'start_at', 'end_at', 'timezone', 'capacity')
        }),
        ('Location', {
            'fields': ('location', 'venue_name', 'address_line_2', 'city', 'country', 'online_url')
        }),
        ('Contact', {
            'fields': ('contact_email', 'contact_phone', 'website')
        }),
        ('Status', {
            'fields': ('state', 'is_public', 'is_free')
        }),
        ('Featured', {
            'fields': ('is_featured', 'feature_requested_at', 'feature_approved_at', 'feature_expires_at', 'feature_order', 'feature_rejection_reason'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
    ...
    def approve_feature(self, request, queryset):
        ...
        updated = queryset.filter(
            feature_requested_at__isnull=False,
            is_featured=False
        ).update(
            is_featured=True,
...
class TicketTypeAdmin(admin.ModelAdmin):
    list_display = ['name', 'event', 'price', 'quantity', 'available_quantity', 'is_active']

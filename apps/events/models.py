...
class EventCustomization(models.Model):
    ...
    event = models.OneToOneField(
        Event, on_delete=models.CASCADE, related_name="customization"
    )
    ...
    layout_template = models.CharField(
        max_length=20, choices=LayoutTemplate.choices, default=LayoutTemplate.DEFAULT
    )
...
class CheckoutQuestion(models.Model):
    ...
    event = models.ForeignKey(
        Event, on_delete=models.CASCADE, related_name="checkout_questions"
    )
...
class Coupon(models.Model):
    ...
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="coupons"
    )
    event = models.ForeignKey(
        Event, on_delete=models.CASCADE, related_name="coupons", null=True, blank=True
    )
    ...
    assignment_type = models.CharField(
        max_length=20, choices=AssignmentType.choices, default=AssignmentType.PUBLIC
    )
...
class CouponUsage(models.Model):
    ...
    booking = models.ForeignKey(
        "tickets.Booking", on_delete=models.CASCADE, related_name="coupon_usages"
    )
    discount_amount = models.DecimalField(max_digits=10, decimal_places=2)
    used_at = models.DateTimeField(auto_now_add=True)
    used_by = models.ForeignKey(
        "core.User", on_delete=models.SET_NULL, null=True, related_name="used_coupons"
    )
...
class EventFlyerConfig(models.Model):
    ...
    event = models.OneToOneField(
        Event, on_delete=models.CASCADE, related_name="flyer_config"
    )
    ...
    photo_shape = models.CharField(
        max_length=10, choices=PhotoShape.choices, default=PhotoShape.CIRCLE
    )
    ...
    photo_bg_color = models.CharField(max_length=30, default="rgba(0,0,0,0)")
...
class FlyerTextField(models.Model):
    ...
    flyer_config = models.ForeignKey(
        EventFlyerConfig, on_delete=models.CASCADE, related_name="text_fields"
    )
    ...
    text_align = models.CharField(
        max_length=10, choices=TextAlign.choices, default=TextAlign.CENTER
    )
    bg_color = models.CharField(max_length=30, default="rgba(0,0,0,0.3)")
...
class FlyerBilling(models.Model):
    ...
    event = models.OneToOneField(
        Event, on_delete=models.CASCADE, related_name="flyer_billing"
    )

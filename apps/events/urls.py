...
urlpatterns = [
    ...
    path('<slug:org_slug>/<slug:event_slug>/tickets/<int:ticket_type_id>/delete/', actions.TicketTypeDeleteView.as_view(), name='delete_ticket_type'),
    path('<slug:org_slug>/<slug:event_slug>/apply-feature/', actions.ApplyFeatureView.as_view(), name='apply_feature'),
    path('<slug:org_slug>/<slug:event_slug>/toggle-publish/', actions.TogglePublishView.as_view(), name='toggle_publish'),
    path('<slug:org_slug>/<slug:event_slug>/toggle-public/', actions.TogglePublicView.as_view(), name='toggle_public'),
    path('<slug:org_slug>/<slug:event_slug>/generate-preview/', actions.GeneratePreviewTokenView.as_view(), name='generate_preview'),

...
class CampayGateway(PaymentGateway):
    ...
    def validate_credentials(self):
        has_username_password = self.credentials.get(
            "app_username"
        ) and self.credentials.get("app_password")
        has_permanent_token = self.credentials.get("permanent_token")
        if not has_username_password and not has_permanent_token:
            raise ValueError(
                "Campay requires either app_username/app_password or permanent_token"
            )
    ...
    def _get_token(self) -> str:
        ...
        try:
            response = requests.post(
                f"{self.host}/api/token/", json=data, headers=headers, timeout=30
            )
    ...
    def initiate_payment(
        ...
        **kwargs,
        ...
        try:
            ...
            withdrawal_fee = self.calculate_withdrawal_fee(amount)
            logger.info(
                f"Campay collect response: {data} (base: {amount}, withdrawal_fee: {withdrawal_fee}, total: {total_amount})"
            )
    ...
    def check_status(self, reference: str) -> PaymentResult:
        ...
        try:
            ...
            if response.ok:
                ...
                return PaymentResult(
                    success=False, status=PaymentStatus.PENDING, message="Request error"
                )

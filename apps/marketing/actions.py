...
class AffiliateListView(LoginRequiredMixin, View):
    def get(self, request, org_slug):
        ...
        links = AffiliateLink.objects.filter(organization=org).annotate(
        ...
        return render(request, 'marketing/affiliate_list.html', {
            'organization': org,
            'links': links,
            'stats': stats,
        })
...
class AffiliateCreateView(LoginRequiredMixin, View):
    def get(self, request, org_slug):
        ...
        return render(request, 'marketing/affiliate_create.html', {
            'organization': org,
            'events': events,
        })
...
class AffiliateDetailView(LoginRequiredMixin, View):
    def get(self, request, org_slug, link_code):
        ...
        conversions = link.conversions.select_related('booking__event').order_by('-created_at')[:50]

        return render(request, 'marketing/affiliate_detail.html', {
            'organization': org,
            'link': link,
    ...
    def post(self, request, org_slug, link_code):
        ...
        return redirect('marketing:affiliate_detail', org_slug=org_slug, link_code=link_code)

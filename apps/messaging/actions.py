...
class CampaignListView(LoginRequiredMixin, View):
    def get(self, request, org_slug):
        ...
        campaigns = MessageCampaign.objects.filter(organization=org).select_related(
            "event"
        )
        return render(
            request,
...
class CampaignCreateView(LoginRequiredMixin, View):
    def get(self, request, org_slug, event_slug):
        ...
        return render(
            request,
            "messaging/campaign_create.html",
            {
                "organization": org,
                "event": event,
                "templates": templates,
...
class CampaignDetailView(LoginRequiredMixin, View):
    def get(self, request, org_slug, campaign_ref):
        ...
        campaign = get_object_or_404(
            MessageCampaign, reference=campaign_ref, organization=org
        )
...
class TemplateCreateView(LoginRequiredMixin, View):
    def get(self, request, org_slug):
        ...
        return render(
            request,
            "messaging/template_create.html",
            {
                "organization": org,
            },
        )

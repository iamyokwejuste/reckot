...
def get_campaign_recipients(campaign):
    ...
    tickets = Ticket.objects.filter(
        booking__event=campaign.event,
        booking__payment__status='CONFIRMED'
...
def prepare_campaign(campaign):
    ...
    for ticket in tickets:
        ...
        delivery = MessageDelivery(
            campaign=campaign,
            ticket=ticket,
            recipient_email=user.email if campaign.message_type == MessageTemplate.Type.EMAIL else '',
            recipient_phone=user.phone_number if campaign.message_type == MessageTemplate.Type.SMS else '',
        )
...
def personalize_message(template, ticket):
    ...
    replacements = {
        ...
        '{{event_date}}': event.start_at.strftime('%B %d, %Y') if event.start_at else '',

// Event Edit Form Alpine Component
(function() {
    'use strict';

    console.log('[EditEvent] Component script loading...');

    const componentFactory = (eventData) => ({
        step: 1,
        totalSteps: 4,
        eventType: eventData.eventType,
        coverPreview: eventData.coverPreview,
    ...
    console.log('[EditEvent] Attempting registration, registerAlpineComponent available:', !!window.registerAlpineComponent);
    console.log('[EditEvent] Alpine available:', typeof Alpine !== 'undefined');

    if (window.registerAlpineComponent) {
        console.log('[EditEvent] Registering via window.registerAlpineComponent');
        window.registerAlpineComponent('editEvent', componentFactory);
    } else {
        console.log('[EditEvent] Registration system not available, waiting...');
        // Fallback: wait for the registration system to be available
        const checkAndRegister = () => {
            if (window.registerAlpineComponent) {
                console.log('[EditEvent] Registration system now available, registering...');
                window.registerAlpineComponent('editEvent', componentFactory);
            } else {
                console.log('[EditEvent] Still waiting for registration system...');
